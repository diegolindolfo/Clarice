...
export default function ModalRenovacao({ emprestimo, onFechar, onConfirmar }: Props) {
  ...
  // Fix timezone offset for formatting
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          {jaRenovado ? (
            ...
            <span className="text-xs font-medium bg-purple-50 text-purple-800 px-3 py-1 rounded-full">
              +15 dias
            </span>
          )}
        </div>
        ...
        {jaRenovado ? (
          <>
            ...
            <button
              onClick={onFechar}
              className="w-full border rounded-xl py-2.5 text-sm hover:bg-gray-50"
            >
              Fechar
            </button>
          </>
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-4 text-sm divide-y divide-gray-100">
              {[
                ...
                <div key={label} className="flex justify-between py-2 first:pt-0 last:pb-0">
                  ...
                  <span className={label === 'Novo prazo' ? 'font-medium text-purple-800' : ''}>
                    {valor}
                  </span>
                </div>
              ))}
