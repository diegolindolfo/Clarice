...
type SalaLeitora = { turma: string; total: number }
...
export default function DashboardPage() {
  ...
  const [salasLeitoras, setSalasLeitoras] = useState<SalaLeitora[]>([])
  ...
  async function carregar() {
    ...
    // Aluno Leitor — top alunos com mais devoluções no período
    const { data: empDevolvidos } = await supabase
      .from('vw_painel_aluno')
      .select('aluno_nome, turma')
      .eq('status', 'DEVOLVIDO')
      .gte('data_saida', dataInicio)

    if (empDevolvidos) {
      const contagem: Record<string, { turma: string; total: number }> = {}
      empDevolvidos.forEach(({ aluno_nome, turma }) => {
        if (!contagem[aluno_nome]) contagem[aluno_nome] = { turma, total: 0 }
        contagem[aluno_nome].total += 1
      })
      const top = Object.entries(contagem)
        .map(([nome, v]) => ({ nome, turma: v.turma, total: v.total }))
        .sort((a, b) => b.total - a.total)
        .slice(0, 5)
      setAlunosLeitores(top)
    }

    // Sala Leitora — turmas com mais devoluções concluídas no período
    const { data: empTurmaDevolvidos } = await supabase
      .from('vw_painel_aluno')
      .select('turma')
      .eq('status', 'DEVOLVIDO')
      .gte('data_saida', dataInicio)

    if (empTurmaDevolvidos) {
      const contagem: Record<string, number> = {}
      empTurmaDevolvidos.forEach(({ turma }) => {
        contagem[turma] = (contagem[turma] ?? 0) + 1
      })
      const ordenado = Object.entries(contagem)
        .map(([turma, total]) => ({ turma, total }))
  ...
  const maxSala = useMemo(() => Math.max(...salasLeitoras.map((t) => t.total), 1), [salasLeitoras])
      ...
      <div className="flex items-center justify-between mb-8 animate-fade-in">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>
            {periodoLabel} · Biblioteca Clarice Lispector
          </p>
        </div>
        <select
            ...
            <div className="grid grid-cols-2 gap-4 animate-slide-up delay-5">
              ...
              <div className="glass-card p-5">
                ...
                {alunosLeitores.length === 0 ? (
                  <p className="text-sm py-4" style={{ color: 'var(--text-muted)' }}>Sem leituras concluídas no período</p>
                ) : (
                  alunosLeitores.map(({ nome, turma, total }, i) => (
                    <div
                      key={nome}
                      className="flex items-center gap-3 py-3"
                      style={{ borderBottom: i < alunosLeitores.length - 1 ? '1px solid var(--border-default)' : 'none' }}
                    >
                      <span
                        className="text-xs font-mono w-5 text-center"
                        style={{
                          color: i === 0 ? '#fbbf24' : i === 1 ? '#a1a1aa' : i === 2 ? '#d97706' : 'var(--text-muted)',
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs truncate" style={{ color: 'var(--text-muted)' }}>{turma}</p>
                      </div>
              ...
              <div className="glass-card p-5">
                ...
                {salasLeitoras.length === 0 ? (
                  <p className="text-sm py-4" style={{ color: 'var(--text-muted)' }}>Sem leituras concluídas no período</p>
                ) : (
                  salasLeitoras.map(({ turma, total }, i) => (
                    <div key={turma} className="flex items-center gap-3 mb-3">
                      <span
